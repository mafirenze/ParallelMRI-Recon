...
def grappa(data, acs, kernel_size=[5, 5], lam=0.01, verbose=True, coils=[], gpu=False):
    ...
    @torch.compile
    def grappa_calibration_matrix(acs):
    ...
    for perdim in permute_dims:
        ...
        AtA = grappa_calibration_matrix(torch.permute(acs, perdim))
        if gpu:
            AtA = AtA.cuda()
        print(AtA.shape)
        ...
        print(kspace.shape, kspace_tmp.shape, perdim)
        print(kspace.device, kspace_tmp.device)
